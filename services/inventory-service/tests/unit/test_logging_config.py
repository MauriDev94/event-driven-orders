...
def test_should_tag_logs_with_inventory_service_name() -> None:
    cap = structlog.testing.LogCapture()
    old_processors = structlog.get_config()["processors"]
    structlog.configure(processors=old_processors[:-1] + [cap])
    try:
        structlog.get_logger("test.logger").info("reserved stock")
    finally:
        structlog.configure(processors=old_processors)

    assert cap.entries[-1]["service"] == "inventory-service"
